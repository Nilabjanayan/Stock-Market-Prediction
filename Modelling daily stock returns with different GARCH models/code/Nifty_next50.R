...
nfty50 <- read.csv('../data/NIFTY NEXT 50_Data.csv')
...
ggplot() + geom_line(mapping = aes(x = 1:length(return), y = return, colour = 'NIFTY 50')) +
  ...
  labs(title = "Log Returns of the Closing Prices of NIFTY NEXT 50", y = "Log Return of the Stock Prices", x = "Time points")
...
##################################################################################
## Model 1: Fit ARMA(2,5)-GARCH(4,0) model with Student t-distribution ########
##################################################################################
n50_garch_1 <- ugarchspec(mean.model = list(armaOrder=c(2,5)),variance.model = list(model = 'sGARCH',garchOrder = c(4,0)),distribution = 'sstd')
