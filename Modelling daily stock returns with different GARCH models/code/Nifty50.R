...
ggplot() + geom_line(mapping = aes(x = 1:length(return), y = return, colour = 'NIFTY 50')) +
  ...
  labs(title = "Log Returns of the Closing Prices of NIFTY 50", y = "Log Return of the Stock Prices", x = "Time points")
...
##################################################################################
## Model 1: Fit ARMA(5,4)-GARCH(1,1) model with Student t-distribution ########
##################################################################################
n50_garch_1 <- ugarchspec(mean.model = list(armaOrder=c(5,4)),variance.model = list(model = 'sGARCH',garchOrder = c(1, 1)),distribution = 'sstd')
